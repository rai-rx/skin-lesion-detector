...
export function AdminDashboard() {
  ...
  const confidenceAverage = stats.totalScans
    ...
    : 0;
  const diagnosisDistribution = stats.diagnosisDistribution.map((item) => ({
    ...item,
    diagnosis: item.diagnosis === 'Squamous Cell Carcinoma' ? 'SCC' : item.diagnosis,
  }));
    ...
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <Card><CardHeader><CardTitle>Confidence Score Distribution</CardTitle><CardDescription>Shows how many scans fall within each model confidence range. This helps identify whether results are generally more certain or concentrated in lower-confidence ranges.</CardDescription></CardHeader><CardContent><div className="h-64">{stats.confidenceDistribution.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={stats.confidenceDistribution}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="bucket" /><YAxis allowDecimals={false} /><Tooltip formatter={(value) => [value, 'Scans']} /><Bar dataKey="count" fill="#2563eb" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <EmptyState />}</div></CardContent></Card>
          <Card><CardHeader><CardTitle>Triage Distribution and Risk Levels</CardTitle><CardDescription>Shows how many scans are classified as low, medium, or high urgency. This provides an overview of the current risk distribution across all scans.</CardDescription></CardHeader><CardContent><div className="h-64">{stats.triageDistribution.some(item => item.count) ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.triageDistribution.filter(item => item.count)} dataKey="count" nameKey="risk" cx="50%" cy="50%" outerRadius={86} label={({ risk, percent }) => `${risk} ${(percent * 100).toFixed(0)}%`}>{stats.triageDistribution.filter(item => item.count).map(item => <Cell key={item.risk} fill={riskColors[item.risk] || riskColors.unknown} />)}</Pie><Tooltip formatter={(value) => [value, 'Scans']} /></PieChart></ResponsiveContainer> : <EmptyState />}</div></CardContent></Card>
          <Card><CardHeader><CardTitle>Daily Activity and Confidence Trends</CardTitle><CardDescription>Compares the number of daily scans with the model's average confidence over time. This makes changes in platform activity and prediction confidence easier to monitor.</CardDescription></CardHeader><CardContent><div className="h-64">{stats.longitudinalTracking.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={stats.longitudinalTracking}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tick={{ fontSize: 11 }} /><YAxis yAxisId="scans" allowDecimals={false} label={{ value: 'Scans', angle: -90, position: 'insideLeft' }} /><YAxis yAxisId="confidence" orientation="right" domain={[0, 100]} label={{ value: 'Confidence %', angle: 90, position: 'insideRight' }} /><Tooltip formatter={(value, name) => [value, name === 'averageConfidence' ? 'Average confidence' : 'Scans']} /><Line yAxisId="scans" type="monotone" dataKey="scans" stroke="#2563eb" strokeWidth={2} /><Line yAxisId="confidence" type="monotone" dataKey="averageConfidence" stroke="#10b981" strokeWidth={2} /></LineChart></ResponsiveContainer> : <EmptyState />}</div><div className="mt-2 flex justify-center gap-4 text-xs text-slate-500"><span className="text-blue-600">Scan volume</span><span className="text-emerald-600">Average confidence</span></div></CardContent></Card>
          <Card><CardHeader><CardTitle>Diagnosis Distribution</CardTitle><CardDescription>Shows how many scans fall into each diagnosis category. This helps admins review the types and volume of classifications recorded by the system.</CardDescription></CardHeader><CardContent><div className="h-64">{diagnosisDistribution.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={diagnosisDistribution} layout="vertical" margin={{ left: 20 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" allowDecimals={false} /><YAxis type="category" dataKey="diagnosis" width={65} /><Tooltip formatter={(value) => [value, 'Scans']} /><Bar dataKey="count" fill="#0f766e" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer> : <EmptyState />}</div></CardContent></Card>
        </div>
